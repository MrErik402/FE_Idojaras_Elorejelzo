...
async function weatherAdd() {
    ...
    if (minField > maxField) {
        ...
        /*----HA LESZ IDŐM IDE CSINÁLJAK EGY SAJÁT CONFIRM ABLAKOS MEGERŐSÍTÉST ARRA, HOGY MEG-E CSERÉLJE A PROGRAM AZ ÉRTÉKEKET----*/
        return;
    }
...
async function weatherSave(){
    ...
    try {
        ...
        if (res.status == 200) {
            ...
            document.querySelector('#maxField').value == "";
            document.querySelector('#weatherType').value == "";
            weatherBTN.classList.remove('hide')
            weatherEditBTN.classList.add('hide')
        }
        showAlert(data.title, data.message, data.type)
        getWeatherDatas()
    } catch (err) {
